...
public class MetodoSimplex {
    ...
    public static Resultado maximizar(double[] c, double[][] A, double[] b) {
        ...
        for (int j = 0; j < n; j++) headers[j] = "x" + (j + 1);
        for (int j = 0; j < m; j++) headers[n + j] = "x" + (n + j + 1);
        headers[cols - 1] = "LD";

        // holguras
        int[] basicas = new int[m];
        for (int i = 0; i < m; i++) basicas[i] = n + i;
        ...
        while (true) {
            ...
            if (colIn == -1) { 
                ...
                r.estado = "OPTIMO";
                r.tablas = tablas; r.basicas = bases; r.headers = headers;
                r.pivotes = pivotes;
                r.z = T[0][cols - 1];
                r.x = new double[n+m];  
            ...
            int rowOut = -1; double mejor = Double.POSITIVE_INFINITY;
            for (int i = 1; i < rows; i++) {
                double a = T[i][colIn];
                if (a > 1e-12) {
                    double ratio = T[i][cols - 1] / a;
                    if (ratio < mejor - 1e-12) { mejor = ratio; rowOut = i; }
                }
            }
            if (rowOut == -1) { 
                Resultado r = new Resultado();
                ...
                r.tablas = tablas; r.basicas = bases; r.headers = headers;
                r.pivotes = pivotes;
                r.x = new double[n+m]; r.z = Double.POSITIVE_INFINITY;
                return r;
            }
    ...
    private static void pivote(double[][] T, int rp, int cp) {
        ...
        double p = T[rp][cp];
        for (int j = 0; j < cols; j++) T[rp][j] /= p;
        for (int i = 0; i < rows; i++) if (i != rp) {
            double f = T[i][cp];
            if (Math.abs(f) > 1e-12) for (int j = 0; j < cols; j++) T[i][j] -= f * T[rp][j];
        }
    }
    ...
    public static String formatearTabla(double[][] T, String[] headers, int[] basicas, int iter) {
        ...
        for (String h : headers) sb.append(String.format("%10s", h));
        sb.append("\n");
        ...
        for (int j = 0; j < headers.length; j++) sb.append(String.format("%10.2f", T[0][j]));
        sb.append("   <- Z\n");
        ...
        for (int i = 1; i < T.length; i++) {
            ...
            for (int j = 0; j < headers.length; j++) sb.append(String.format("%10.2f", T[i][j]));
            sb.append("\n");
        }
